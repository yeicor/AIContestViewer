...
func _on_game_state(_state: GameState, turn: int, phase: int):
	...
	if phase == SignalBus.GAME_STATE_PHASE_END_ROUND:
		...
		var sorted_player_names := UI.player_names_by_total_score()
		for player in players_node.get_children():
			var pl := player as PlayerScene
			var order := sorted_player_names.find(pl.name)
			assert(order >= 0)
...
func _animate_player(pl: PlayerScene, order: int, podium: Podium):
	...
	var walk_time: float = min(3.0, Settings.common_end_round_secs() / 3.0)
	pl.walk_to_3d(final_t.origin, walk_time)
	await get_tree().create_timer(walk_time).timeout
	#var dance_time := Settings.common_end_round_secs() - walk_time
